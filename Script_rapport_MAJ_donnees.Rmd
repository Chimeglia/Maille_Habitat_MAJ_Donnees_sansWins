...
knit: (function(inputFile, encoding) { 
  rmarkdown::render(inputFile,
                    encoding=encoding, 
                    output_file='Rapport_Etude_Maille_Habitat_MAJ_Données.html') })
---
...
Indicateurs_parCommune= Donnees_Logement %>% 
  mutate(indic.transac = round(100 * nb_mut / LOG, 1),
         ...
         indic.prop = round(100 * proprietaire / RP_Filocom, 1),
         indic.duroc = duroc,
...
Les précédents indicateurs sont mis à jour avec des données plus récentes et de nouveaux indicateurs sont ajoutés.
...
<br>
<strong>
Nombre de transactions rapporté au nombre de logements <span style="color:red">(Source : DV3F 2010-2017)</span>  
</strong>

<span style="color:red"><strong>
Attention : Les données n'ont pas été mises à jour pour le moment.
</strong>
</span>
...
<br>
<strong>
Taux de logements occupés par leur propriétaire (Source : FILOCOM) 
</strong>
...
<strong>
Cette donnée est également présente dans le recensement INSEE sans secrétisation. Il peut être pertinent d'utiliser cette source.
</strong>
</span>
...
<br>
<strong>
Prix du mètre carré dans l’ancien rapporté au revenu fiscal médian communal (Source : DV3F, FILOCOM)
</strong>
...
## &nbsp; Sans l'indicateur "nombre de logements"

<br>

### &nbsp;&nbsp; Mêmes données mises à jour
...
### &nbsp;&nbsp; Ajout du "revenu total Airbnb" + MAJ données 
...
### &nbsp;&nbsp; Ajout du "revenu moyen / logement Airbnb" + MAJ données 
...
# Création d'une palette de couleurs pour les secteurs
palSecteurs = colorFactor(palette = c("seagreen3", "orange2","blueviolet", "deeppink"), domain = carteMaillesSecteurs$cl_maille4)
palSecteurs = colorFactor(palette = c("deeppink", "blueviolet","orange2", "seagreen3"), domain = carteMaillesSecteurs$cl_maille4)
...
## &nbsp; Avec l'ajout de l'indicateur "nombre de logements"

### &nbsp;&nbsp; Ajout du "nombre de logements"
...
# Création d'une palette de couleurs pour les secteurs
palSecteurs = colorFactor(palette = c("seagreen3", "orange2","blueviolet", "deeppink"), domain = carteMaillesSecteurs$cl_maille4)
palSecteurs = colorFactor(palette = c("blueviolet", "deeppink","seagreen3", "orange2"), domain = carteMaillesSecteurs$cl_maille4)
...
### &nbsp;&nbsp; Ajout du "revenu total Airbnb" + "nombre de logements" + MAJ données 
...
# Création d'une palette de couleurs pour les secteurs
palSecteurs = colorFactor(palette = c("seagreen3", "orange2","blueviolet", "deeppink"), domain = carteMaillesSecteurs$cl_maille4)
palSecteurs = colorFactor(palette = c("deeppink", "blueviolet","seagreen3", "deeppink"), domain = carteMaillesSecteurs$cl_maille4)
...
### &nbsp;&nbsp; Ajout du "revenu moyen / logement Airbnb" + "nombre de logements" + MAJ données 
...
Donnees_Cube1=Donnees_Cube1 %>% select(CODGEO,LIBGEO,tot_lgmt,jeun,anc,nb_mut,moyenne_prix,rev_ucm,RP_Filocom=RP.y)



Donnees_proprietaire=ImportFILOCOM_cube3  %>% 
                    left_join(ImportINSEE,by="CODGEO") %>% 
                    select(CODGEO, PO, totalRP,`Part des propriétaires dans les RP`)
Donnees_proprietaire[, 2:ncol(Donnees_proprietaire)] <- Donnees_proprietaire[, 2:ncol(Donnees_proprietaire)] %>%  mutate_all(as.numeric)
Donnees_proprietaire=Donnees_proprietaire %>% 
                    mutate(proprietaire=ifelse(is.na(PO), `Part des propriétaires dans les RP`, PO)) %>% 
                    select(CODGEO, proprietaire)
...
Donnees_INSEE=baseLogementINSEE_Corse_2019 %>% 
              ...
              select(CODGEO,POP=P19_POP,LOG=P19_LOG, RP=P19_RP, RSECOCC=P19_RSECOCC, LOGVAC=P19_LOGVAC,RP_PROP=P19_RP_PROP)


Donnees_Logement_MAJ=Donnees_Cube1 %>% 
  left_join(Donnees_INSEE,by="CODGEO")  %>% 
  left_join(Donnees_proprietaire,by="CODGEO") %>% 
